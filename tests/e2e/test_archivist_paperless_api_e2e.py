...
go red first and the unit fixtures in ``tests/unit/stacklets/test_pipeline.py``
...
async def bot_api(paperless):
    ...
    ``tests/e2e/paperless.py``: the question these tests answer
...
class TestTaskApiShape:
    ...
    ``tests/unit/stacklets/test_pipeline.py`` parses task payloads offline, so
