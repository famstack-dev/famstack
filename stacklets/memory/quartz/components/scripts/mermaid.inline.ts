...
document.addEventListener("nav", async () => {
  ...
  // OVERLAY — upstream imports mermaid straight from
  // cdnjs.cloudflare.com here. That request only fires on a page that
  // actually contains a diagram, which is why the demo vault never
  // triggered it and nobody noticed: the guarantee was a property of
  // the *content*, not of the build. The first family note with a
  // flowchart in it would have started calling Cloudflare.
  //
  // The Dockerfile installs mermaid at the same pinned version and
  // copies its minified ESM graph into static/mermaid, so this loads
  // from our own host. The chunks import each other by relative path,
  // so the entry file is the only URL that needs rewriting.
  //
  // The specifier goes through a variable deliberately: esbuild bundles
  // dynamic imports whose specifier is a string literal, and a literal
  // "/static/…" would be resolved against the filesystem at build time
  // and fail the build. Through a variable it stays a runtime import.
